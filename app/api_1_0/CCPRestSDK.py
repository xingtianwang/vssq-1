...
class REST(object):
    ...
    def CreateSubAccount(self, friendlyName):
        ...
        m = hashlib.md5()
        m.update(bytes(signature, 'utf-8'))
        ...
        auth = base64.b64encode(bytes(src, 'utf-8')).strip()
        ...
        data = ''
        req.data = bytes(body, encoding='utf-8')
        try:
    ...
    def querySubAccount(self, friendlyName):
        ...
        # 生成auth
        src = self.AccountSid + ":" + self.Batch
        auth = base64.b64encode(src).strinp()
        req = urllib.request.Request(url)
        self.setHttpHeader(req)
...
#                 self.log(url,body,data)
    ...
    def accAuth(self):
        if(self.ServerIP == ""):
            print('172004')
            print('IP为空')
        if(not self.ServerPort.isalnum()):
            print('172005')
            print('端口错误（小于等于0）')
