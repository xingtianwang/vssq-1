...
from .models import GodinAccount, DeviceInfo, UserInfo, \
    BindRecord, FeedBack, AppVersion, AppList, ExceptionLog, Activity, \
    OpenScreenAds, OpenScreenAdsStatistics, BannerAds, BannerAdsStatistics, \
    OpenScreenSimulateData, OpenScreenSimulatedUser, VipMembers, MemberWare, MemberWareOrder, \
    CommunicationGroup, OpenScreenAdsData, InteractiveAds, InteractiveAdsStatistics, \
    ...
    Free_Experience_Days

from .sms import SmsWwtl
from .utils import get_white_imei_list, gen_key_order_num, create_key, gen_avatar_vip_order_num, \
    feature_file, get_imei, get_app_version_white, gen_business_vip_order_num
from .apktool.conversion_apk import format_apk
...
from .utils import validate_client, validate_app_exception
...
class RegisterApi(Resource):
    ...
    def post(self):
        ...
        if rv is None:
            # print_warn(action='Response', function=self.__class__.__name__, branch='SMS_EXPIRED',
            #            api_version='v1.0', imei=json_req['imei'])
        ...
        # 校验从redis取出的手机验证码和前端传来的手机验证码是否相等
            ...
            # 设备信息 with_lock ?
            new_device_info = DeviceInfo().query.with_lockmode("update").filter_by(imei=json_req['imei']).limit(1).first()
            # 设备信息表是否有该手机信息
            # 如果没有
            ...
            bind_record = BindRecord().query.filter_by(godin_id=user_info.godin_id, imei=user_info.imei).limit(1).first()
            ...
            # print_error(action='Response', function=self.__class__.__name__, branch='SMS_INVALID',
            #             api_version=g_api_version, imei=json_req['imei'])
...
class GetUserVipStatusApi(Resource):
    ...
    def post(self):
        ...
        if act_member:
            activate = 1
        vip_member = VipMembers.query.filter_by(godin_id=json_req['godin_id']).filter_by(status=1).limit(1).first()
        if vip_member is None:
            return get_response({'head': ErrorCode.USER_NOT_VIP, 'body': {'activate': activate}})
        else:
            ...
            if not vip_member.valid_time:
                ...
                if vip_member.valid_time < datetime.datetime.now() and vip_member.status == 1:
                    ...
                    return get_response({'head': ErrorCode.USER_NOT_VIP, 'body': {'activate': activate, "grade": activate}})
...
class GetChannelVipWareApi(Resource):
    ...
    def post(self):
        ...
        wares = MemberWare.query.filter(MemberWare.channel.in_([json_req['channel'], 'moren']),MemberWare.gold_or_platinum==1).filter_by(status=1)
        ...
        for ware in wares:
            if ware.channel == json_req['channel']:
                ...
                cell = {
                    ...
                    'discount_price': float(ware.price * ware.discount),
        ...
        for m_ware in wares:
            if m_ware.channel == 'moren':
                if m_ware.category not in ware_cate:
                    if m_ware.picture != '':
                        picture = current_app.config['FILE_SERVER'] + m_ware.picture
                    else:
                        picture = ''
                    cell = {
                        'id': m_ware.id,
                        'name': m_ware.name,
                        'category': m_ware.category,
                        'days': type_dict[m_ware.category],
                        'price': m_ware.price,
                        'discount': float(m_ware.discount),
                        'discount_price': float(m_ware.price * m_ware.discount),
                        'priority': m_ware.priority,
                        'description': m_ware.description,
                        'channel': m_ware.channel,
                        'picture': picture
                    }
                    ware_list.append(cell)
        # print_info(action='Response', function=self.__class__.__name__, branch='SUCCESS',
...
class GetUserVipOrderApi(Resource):
    ...
    def post(self):
        ...
        for order in MemberWareOrder.query.filter(MemberWareOrder.buyer_godin_id == json_req['godin_id'],
            ...
            ware_order.append(order.to_json(category, type_name))
        # print_info(action='Response', function=self.__class__.__name__, branch='SUCCESS',
        #            api_version=g_api_version, imei=json_req['imei'])
...
class BuyVipWareApi(Resource):
    ...
    def post(self):
        ...
        if ware_info is None:
            # print_warn(action='Response', function=self.__class__.__name__, branch='WARE_NOT_EXIST',
            #            api_version=g_api_version, imei=json_req['imei'])
...
class ActivateVipMemberApi(Resource):
    ...
    def post(self):
        ...
        #     order.discount = ware.discount
        #     order.discount_price = ware.price * ware.discount
        #     order.pay_type = 2
        #     order.status = 1
        #     order.category = 2
        #     if act.vip_type == 1:
        ...
        #     channel = act.channel
        #     i += 1
        # last_order = MemberWareOrder.query.filter_by(buyer_godin_id=json_req['godin_id'], status=1). \
        #     order_by(MemberWareOrder.pay_time.desc()).limit(1).first()
        # vip_member = VipMembers.query.filter_by(godin_id=json_req['godin_id']).limit(1).first()
        # if last_order is not None:
        #     vip_member.valid_time = last_order.end_time
...
class GetInteractiveAdsApi(Resource):
    ...
    def post(self):
        ...
        if imei_vip is not None and imei_vip.valid_time >= datetime.datetime.now().date():
            res = []
            # print_info(action='imeivip', function=self.__class__.__name__, branch="vip", api_version=g_api_version,
            #            imei=json_req['imei'])
        else:
...
class GetBanneradsApi(Resource):
    ...
    def post(self):
        ...
        imei_vip = ImeiVip.query.filter_by(imei=json_req['imei']).limit(1).first()
        if imei_vip is not None:
            if imei_vip.valid_time >= datetime.datetime.now().date():
                infos = infos.filter(BannerAds.source == 0).all()
        data = []
        flag = 0
        current_time = datetime.datetime.now()
        r_time = str(current_time.year) + '-' + str(current_time.month) + '-' + str(current_time.day) + " "
...
class JudgeAddKeyApi(Resource):
    ...
    def get(self):
        ...
        # from flask import request
        # ip = request.headers.get('X-Forwarded-For')
        # print('judge: ', ip, json_req['imei'])

        user_record = UserKeyRecord.query.filter_by(imei=json_req['imei'], status=1).limit(1).first()
        if user_record is None:
            # print_info(action='Response', function=self.__class__.__name__, branch='ENTER_PAGE',
            #            api_version=g_api_version, imei=json_req['imei'])
...
class BuykeyApi(Resource):
    ...
    def post(self):
        ...
        if user_record is not None:
            # print_error(action='Response', function=self.__class__.__name__, branch='KEY_EXIST',
            #             api_version=g_api_version, imei=json_req['imei'])
...
class CheckChannelAppUpdateApi(Resource):
    ...
    def post(self):
        ...
        if app_version is None:
            # print_error(action='Response', function=self.__class__.__name__, branch='APP_NOT_EXIST',
            #             api_version=g_api_version, imei=json_req['imei'])
...
class GetNoticeApi(Resource):
    ...
    def get(self):
        ...
        query = SysNotice.query.filter(SysNotice.flag_id > json_req['flag_id'], SysNotice.status == 1,
        ...
        if len(user_record) <= 0:
            ...
            if key.status == 0:
                for imei in imei_list:
                    ...
                    if key_record is not None:
                        vip_ad_time = key_record.vip_ad_time
                    imei_vip = ImeiVip.query.filter_by(imei=imei).limit(1).first()
                    if imei_vip is None:
                        imei_vip = ImeiVip()
                        imei_vip.imei = imei
                        imei_vip.create_time = datetime.datetime.now()
...
class GetOpenAdsInfoApi(Resource):
    ...
    def post(self):
        ...
        if not res:
            ...
            return {"head": ErrorCode.SUCCESS, "body": res}
